...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    const releaseMetadataURI = `ipfs://${await uploadToIPFS(JSON.stringify(METADATA.release), true)}`;
    const buildMetadataURI = `ipfs://${await uploadToIPFS(JSON.stringify(METADATA.build), true)}`;
    ...
    // Get PluginRepo
    const pluginRepo = PluginRepo__factory.connect(getPluginInfo(network.name)[network.name]['address'], deployer);
    ...
    if (VERSION.build > latestBuild + 1) {
        throw Error(
            `Publishing with build number ${VERSION.build} is not possible. 
      The latest build is ${latestBuild} and the next release you can publish is release number ${latestBuild + 1}.`
        );
    }
    ...
    const implementationAddress = await PluginSetup__factory.connect(setup.address, deployer).implementation();
