...
import { findEventTopicLog, addDeployedRepo as addCreatedRepo, getPluginRepoFactoryAddress } from '../../utils/helpers';
import { PluginRepoFactory__factory, PluginRepoRegistry__factory, PluginRepo__factory } from '@aragon/osx-ethers';
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    const pluginRepoFactoryAddr: string = getPluginRepoFactoryAddress(network.name);
    // console.log(pluginRepoFactoryAddr);

    const pluginRepoFactory = PluginRepoFactory__factory.connect(pluginRepoFactoryAddr, deployer);

    const tx = await pluginRepoFactory.createPluginRepo(PLUGIN_REPO_ENS_NAME, deployer.address);
    ...
    addCreatedRepo(network.name, PLUGIN_REPO_ENS_NAME, pluginRepo.address, [], blockNumberOfDeployment);
};
