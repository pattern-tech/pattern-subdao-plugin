import { PluginRepo, MyPluginSetup, MyPluginSetup__factory } from '../../typechain';
...
describe('PluginRepo Deployment', function () {
    ...
    before(async () => {
        ...
        pluginRepo = PluginRepo__factory.connect(getPluginInfo('hardhat')['hardhat'].address, alice);
    });
    ...
    context('PluginSetup Publication', async () => {
        ...
        before(async () => {
            setup = MyPluginSetup__factory.connect((await deployments.get('MyPluginSetup')).address, alice);
        });
