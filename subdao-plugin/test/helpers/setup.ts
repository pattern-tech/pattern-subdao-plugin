...
export async function installPLugin(
    ...
    const preparedEvent = await findEvent<InstallationPreparedEvent>(prepareTx, 'InstallationPrepared');
    if (!preparedEvent) {
    ...
    const appliedEvent = await findEvent<InstallationAppliedEvent>(applyTx, 'InstallationApplied');
    if (!appliedEvent) {
...
export async function uninstallPLugin(
    ...
    const preparedEvent = await findEvent<UninstallationPreparedEvent>(prepareTx, 'UninstallationPrepared');
    if (!preparedEvent) {
    ...
    const appliedEvent = await findEvent<UninstallationAppliedEvent>(applyTx, 'UninstallationApplied');
    if (!appliedEvent) {
