...
describe(PLUGIN_CONTRACT_NAME, function () {
    ...
    describe('initialize', async () => {
        it('reverts if trying to re-initialize', async () => {
            await expect(myPlugin.initialize(dao.address, defaultInput.number)).to.be.revertedWith(
    ...
    describe('storeNumber', async () => {
        ...
        it('reverts if sender lacks permission', async () => {
            await expect(myPlugin.connect(bob).storeNumber(newNumber))
                ...
                .withArgs(dao.address, myPlugin.address, bob.address, STORE_PERMISSION_ID);
        });
        ...
        it('emits the NumberStored event', async () => {
            await expect(myPlugin.storeNumber(newNumber)).to.emit(myPlugin, 'NumberStored').withArgs(newNumber);
        });
    });
});
