...
import { DAO, MyPluginSetup, MyPluginSetup__factory, MyPlugin__factory } from '../../typechain';
import { deployTestDao } from '../helpers/test-dao';
import { Operation, getNamedTypesFromMetadata } from '../helpers/types';
import { defaultInitData } from './simple-storage';
import { ADDRESS_ZERO, EMPTY_DATA, NO_CONDITION, STORE_PERMISSION_ID, abiCoder } from './simple-storage-common';
...
describe(PLUGIN_SETUP_CONTRACT_NAME, function () {
    ...
    describe('prepareInstallation', async () => {
        ...
        before(async () => {
            initData = abiCoder.encode(
                getNamedTypesFromMetadata(buildMetadata.pluginSetup.prepareInstallation.inputs),
                [defaultInitData.number]
            );
        });

        it('returns the plugin, helpers, and permissions', async () => {
            const nonce = await ethers.provider.getTransactionCount(myPluginSetup.address);
            ...
            } = await myPluginSetup.callStatic.prepareInstallation(dao.address, initData);

            expect(plugin).to.be.equal(anticipatedPluginAddress);
            expect(helpers.length).to.be.equal(0);
            expect(permissions.length).to.be.equal(1);
            expect(permissions).to.deep.equal([
                [Operation.Grant, plugin, dao.address, NO_CONDITION, STORE_PERMISSION_ID],
            ]);
    ...
    describe('prepareUninstallation', async () => {
        it('returns the permissions', async () => {
            ...
            const permissions = await myPluginSetup.callStatic.prepareUninstallation(dao.address, {
                plugin: dummyAddr,
                currentHelpers: [],
                data: EMPTY_DATA,
            });
