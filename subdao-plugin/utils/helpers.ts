...
export function getPluginRepoFactoryAddress(networkName: string) {
    ...
    if (networkName === 'localhost' || networkName === 'hardhat' || networkName === 'coverage') {
        const hardhatForkNetwork = process.env.NETWORK_NAME ? process.env.NETWORK_NAME : 'mainnet';
        ...
        pluginRepoFactoryAddr = osxContracts[networkNameMapping[networkName]].PluginRepoFactory;
...
export function getPluginInfo(networkName: string): any {
    ...
    if (existsSync(pluginInfoFilePath) && statSync(pluginInfoFilePath).size !== 0) {
...
function storePluginInfo(networkName: string, pluginInfo: any) {
    if (['localhost', 'hardhat', 'coverage'].includes(networkName)) {
        writeFileSync('plugin-info-testing.json', JSON.stringify(pluginInfo, null, 2) + '\n');
    } else {
...
export function addCreatedVersion(
    ...
    pluginInfo[networkName]['releases'][version.release]['releaseMetadataURI'] = metadataURIs.release;

    pluginInfo[networkName]['releases'][`${version.release}`]['builds'][`${version.build}`] = {};
