...
if (meta.main) {
  ...
  const config: ArgumentConfig<MyArgs> = {
    ...
    network: {
      ...
      alias: "n",
      description:
        "The network, can be one of [mainnet, goerli, polygon, mumbai, base, baseGoerli, local]",
    },
  };
...
export async function installSubDaoPlugin(
  childDAO: string,
  parentDAO: string,
  ...
  const createProposalStep2Value = await (
    await createProposalSteps.next()
  ).value;
